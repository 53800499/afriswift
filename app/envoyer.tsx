...
export default function EnvoyerScreen() {
  ...
  return (
    <View style={{ flex: 1, backgroundColor: "#f9f9f9" }}>
      <Header title={"Envoyer de l'argent"} showBackButton rightContent={<TouchableOpacity><Ionicons name="help-circle-outline" size={22} color="#888" /></TouchableOpacity>} />
      <ScrollView contentContainerStyle={{ padding: 20, paddingBottom: 40 }}>
        {/* Méthode de paiement */}
        <View style={styles.card}>
          <Text style={styles.label}>Méthode de paiement</Text>
          <View style={{ flexDirection: "row", gap: 10, marginTop: 10 }}>
            <TouchableOpacity style={[styles.methodBtn, methode === "mobile" && styles.methodBtnActive]} onPress={() => setMethode("mobile")}> 
              <Text style={[styles.methodBtnText, methode === "mobile" && styles.methodBtnTextActive]}>Mobile Money</Text>
            </TouchableOpacity>
            <TouchableOpacity style={[styles.methodBtn, methode === "bank" && styles.methodBtnInactive]} onPress={() => setMethode("bank")}> 
              <Text style={[styles.methodBtnText, methode === "bank" && styles.methodBtnTextActive]}>Compte Bancaire</Text>
            </TouchableOpacity>
          </View>
        </View>
        ...
        <View style={styles.card}>
          ...
          <View style={styles.inputRow}>
            <TextInput
              ...
              selectTextOnFocus
            />
            <TouchableOpacity style={styles.inputIcon}><Ionicons name="qr-code-outline" size={22} color="#0a7e3a" /></TouchableOpacity>
          </View>
          <View style={{ flexDirection: "row", gap: 16, marginTop: 8 }}>
            <TouchableOpacity style={styles.linkRow}><Ionicons name="people-outline" size={16} color="#0a7e3a" /><Text style={styles.linkText}>Contacts récents</Text></TouchableOpacity>
            <TouchableOpacity style={styles.linkRow}><Ionicons name="clipboard-outline" size={16} color="#0a7e3a" /><Text style={styles.linkText}>Coller</Text></TouchableOpacity>
          </View>
        </View>
        ...
        <View style={styles.cardDestinataire}>
          ...
          <View style={styles.destHeader}>
            <View style={styles.avatarLarge}><Ionicons name="person" size={28} color="#888" /></View>
        ...
        <View style={styles.cardMontant}>
          ...
          <View style={styles.montantRow}>
            ...
            <Text style={styles.montantEur}>≈ {(Number(montant || 0) * 0.001524).toFixed(2)} EUR</Text>
          </View>
          {/* Clavier numérique */}
          <View style={styles.keypadBlock}>
            {["1","2","3","4","5","6","7","8","9",".","0","del"].map((k, i) => (
              <TouchableOpacity key={i} style={styles.keyBtn} onPress={() => handleKey(k)}>
                {k === "del" ? <Ionicons name="arrow-back" size={22} color="#0a7e3a" /> : <Text style={styles.keyBtnText}>{k}</Text>}
              </TouchableOpacity>
            ))}
          </View>
          <View style={styles.fraisBlock}>
            ...
            <View style={styles.fraisRow2}>
              ...
              <Text style={styles.fraisTotal2}>{total.toLocaleString()} FCFA</Text>
            </View>
          </View>
        </View>
        {/* Message sécurité */}
        <Text style={styles.securite2}>Transaction sécurisée et cryptée. Temps estimé: 30 secondes</Text>
        {/* Bouton envoyer */}
